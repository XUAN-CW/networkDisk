...
do
    ########### 找到一个符合要求的文件 #################################
    shouldBeUploaded=$(find . -type f -size +90M -size -20G 2>/dev/null|sed $'s/\([\'\"]\)/\1\\\\\'/g'|sed 's/\(.*\)/\"\1\"/'| xargs du --exclude="." -k 2>/dev/null| grep -v '!qB$'| sort -n | sed -n 1p | awk 'sub($1,"")'|awk '$1=$1')
    ...
    if [[ ${empty} = ${shouldBeUploaded} ]] ; then
        ...
        shouldBeUploaded="${shouldBeUploaded:2}"
    fi
    #################################################################
#    echo "shouldBeUploaded:${shouldBeUploaded}"
    dir="${shouldBeUploaded%.*}[dir-$(date +%s)]"
    mkdir "${dir}"
#    echo "dir:${dir}"
